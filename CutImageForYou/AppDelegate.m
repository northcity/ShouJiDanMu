...
//                [gameScore setObject:@"关" forKey:@"DanMuZhuShouDuLiApp"];
...
//                }];
    ...
    if ([nowStatus isEqualToString:@"开"]) {
        ...
        [bquery getObjectInBackgroundWithId:@"867f548dbb" block:^(BmobObject *object,NSError *error){
            if (error){
                //进行错误处理
            }else{
                //表里有id为0c6db13c的数据
                if (object) {
                    ...
                    NSString *KaiGuanStatus = [object objectForKey:@"DanMuZhuShouDuLiApp"];
                    NSLog(@"%@=========",KaiGuanStatus);
