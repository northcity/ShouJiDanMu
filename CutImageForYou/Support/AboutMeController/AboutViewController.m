...
//        [cell.contentView.layer insertSublayer:subLayerIcon below:imageViewPingGuo.layer];
    ...
    if (indexPath.row == 5) {
        if (!_shiJianJiaoNangBtn) {
            _shiJianJiaoNangBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            _shiJianJiaoNangBtn = [self CellContentViewwWithBtnNormalString:@"打开" WithBtnSelectedString:@"获取" WithAppUrlSecheme:@"chenxishijianjiaonang://" WithImageName:@"zuizhongIcon.png" WithTitleString:@"时间胶囊" WithDetailString:@"乘时间之海" WithCell:cell];
            [_shiJianJiaoNangBtn addTarget:self action:@selector(awakeShiJianJiaoNangApp) forControlEvents:UIControlEventTouchUpInside];
        }
    ...
    if (indexPath.row == 7) {
        if (!_jiuGongGeBtn) {
           _jiuGongGeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            _jiuGongGeBtn = [self CellContentViewwWithBtnNormalString:@"打开" WithBtnSelectedString:@"获取" WithAppUrlSecheme:@"comchenxijiugongge://" WithImageName:@"iconJiu.png" WithTitleString:@"九宫格切图" WithDetailString:@"极简主义" WithCell:cell];
            [_jiuGongGeBtn addTarget:self action:@selector(awakeJiuGongGeApp) forControlEvents:UIControlEventTouchUpInside];
        }
    }
    ...
    if (indexPath.row == 6) {
        if (!_shouchiDanMuBtn) {
            _shouchiDanMuBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            _shouchiDanMuBtn = [self CellContentViewwWithBtnNormalString:@"去评价" WithBtnSelectedString:@"获取" WithAppUrlSecheme:@"comchenshouchidanmu://" WithImageName:@"icon.png" WithTitleString:@"Version:1.0" WithDetailString:@"无广告手持LED弹幕" WithCell:cell];
            [_shouchiDanMuBtn addTarget:self action:@selector(awakeSelfApp) forControlEvents:UIControlEventTouchUpInside];
        }
...
- (void)awakeSelfApp{
    NSString *itunesurl = @"itms-apps://itunes.apple.com/cn/app/id1390206197?mt=8&action=write-review";
